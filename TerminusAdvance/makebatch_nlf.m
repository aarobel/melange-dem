...
shrt = ['_nlf_h' int2str(10*hsi) '_str' num2str(str/1e5) 'e5_lfc' int2str(1000*lfc)];

fid = fopen(['batch_' runname '.sh'],'w');

fprintf(fid,'%s\n','#!/bin/bash');
fprintf(fid,'%s\n',['#SBATCH --job-name=bcnlh' int2str(10*hsi) 'sb' num2str(str/1e5)]);
...
fprintf(fid,'%s\n',['matlab -nodesktop -nosplash -r "chanbonds_fromstop_nlf(' sprintf('\''') run sprintf('\''') ',$1,$2,' num2str(hsi) ',' int2str(str) ',' num2str(lfc) ');makeLAMMPSrun_nlf($2,' sprintf('\''') run sprintf('\''') ',' num2str(hsi) ',' int2str(str) ',' num2str(lfc) ');quit;"']);
