function makeLAMMPSrun_nolf(iter,run,hsi,str,lfc)
...
%if(iter==1)
	...
	fprintf(fid,'%s\n','variable        trun    equal  (10*24*3600/${dt})-${trun1}');
%else
%	fprintf(fid,'%s\n','variable        trun    equal  10*24*3600/${dt}');
%end
...
if(iter==1)
	fprintf(fid,'%s\n',['read_data       ./init/fjordbond_lgberg_nolf_mbtr0_dmax10.init']);	
else
...
fprintf(fid,'%s\n','variable        xtbot        equal bound(all,xmin)+(${termvel}*${trun}*${dt}*(${iter}-1))');
fprintf(fid,'%s\n','variable        xtop         equal bound(all,xmax)');
fprintf(fid,'%s\n','variable        xtermbot     equal ${xtbot}+210');
...
fprintf(fid,'%s\n',['fix             lfc1 noterm bond/create/gran/disk ${bondupd} 1 1 10.0 1 40 prob ' num2str(lfc) ' 32903']);
%fprintf(fid,'%s\n\n',['fix             lfc2 noterm bond/create/gran/disk ${bondupd} 1 2 10.0 1 40 prob ' num2str(lfc) ' 32903']);
